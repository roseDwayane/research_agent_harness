...
def test_full_pipeline_then_replay_is_byte_identical(tmp_path: Path, offline):
    ...
    raw = json.loads(s.path("step2_raw_papers.json").read_text())
    ...
    sl = json.loads(s.path("step3_shortlist.json").read_text())
    ...
    allrec = json.loads(s.path("step3_screening_all.json").read_text())
    ...
    md3 = s.path("step3_screening_results.md").read_text()
    assert "Checkpoint 2" in md3 and "Population mismatch" in md3

    bib = s.path("step4_references.bib").read_text()
    keys = json.loads(s.path("step4_citation_keys.json").read_text())
    assert len(re.findall(r"^@", bib, re.M)) == len(sl["papers"]) == len(keys)
    apa = s.path("step4_references_apa.md").read_text()
    assert "Lee, A., Chan, B., & Dee, C. (2023)." in apa

    log = s.path("step5_full_text/_access_log.md").read_text()
    assert "full-text-html" in log
    d_key = keys[d["id"]]["key"]
    dmd = s.path(f"step5_full_text/{d_key}.md").read_text()
    assert 'access_level: "full-text-html"' in dmd and "extraction_method" in dmd and "## 2 Methods" in dmd
    assert "summarized" not in log

    canvas = json.loads(s.path("step6_knowledge_graph.canvas").read_text())
    ...
    gaps = json.loads(s.path("step7_gaps.json").read_text())
    ...
    md7 = s.path("step7_gap_analysis.md").read_text()
    assert "5 × 0.40 + 4 × 0.30 + 3 × 0.30 = 2.00 + 1.20 + 0.90 = **4.10**" in md7

    cps = json.loads(s.path("checkpoints.json").read_text())["decisions"]
    assert cps["3"]["decision"]["lock"] == "GAP_001" and cps["4"]["decision"]["approved"] is True
    hyp = s.path("step8_hypothesis_specification.md").read_text()
    assert 'selected_gap: "GAP_001"' in hyp and "dropped_gaps:" in hyp

    intro = s.path("step9_manuscript/01_intro.tex").read_text()
    ...
    pruned = s.path("step9_manuscript/references.bib").read_text()
    cited = set(re.findall(r"\\cite\{([^}]*)\}", intro + s.path("step9_manuscript/02_relatedwork.tex").read_text()))
    ...
    val = json.loads(s.path("step9_manuscript/_validation.json").read_text())
    assert set(val["phantom_removed"]) == {"Phantom2099", "Fake2000"}

    manifest = json.loads(s.path("run_manifest.json").read_text())
...
def test_checkpoint2_rescue_and_file_decisions(tmp_path: Path, offline):
    ...
    assert border[0] in json.loads(s.path("step4_citation_keys.json").read_text())
    assert json.loads(s.path("checkpoints.json").read_text())["decisions"]["2"]["decision"]["include"] == [border[0]]
